...
void FRenderer::Create(HWND WindowHandle)
{
...
void FRenderer::Resize(UINT InWidth, UINT InHeight)
{
...
ID3D11Buffer* FRenderer::CreateIndexBuffer(UINT* Indices, UINT Count)
{
...
void FRenderer::ReleaseIndexBuffer(ID3D11Buffer* IndexBuffer)
{
	IndexBuffer->Release();
}

TSharedPtr<FRenderPipeline> FRenderer::CreateRenderPipeline()
{
...
void FRenderer::UpdateModelConstant(FMatrix Model)
{
	DefaultPipeline->UpdateConstantBuffer(0, FConstants{ Model });
}

void FRenderer::UpdateViewConstant(FMatrix View)
{
	DefaultPipeline->UpdateConstantBuffer(1, FConstants{ View });
}

void FRenderer::BindPipeline(const TSharedPtr<FRenderPipeline>& Pipeline) const
{
...
void FRenderer::RenderPrimitive(const TSharedPtr<FRenderPipeline>& Pipeline, ID3D11Buffer* Buffer, UINT NumVertices) const
{
...
void FRenderer::RenderPrimitive(ID3D11Buffer* Buffer, UINT NumVertices, const FMatrix& Model) const
{
...
void FRenderer::RenderPrimitive(ID3D11Buffer* Buffer, UINT NumVertices, const FMatrix& Model, const FVector4& Color) const
{
...
void FRenderer::RenderLine2D(const FVector2& Start, const FVector2& End, const FVector4& Color, float Thickness) const
{
...
void FRenderer::RenderCircle2D(const FVector2& Center, const FVector4& Color, float Radius) const
{
...
void FRenderer::RenderTriangle2D(const FVector2& Center, const FVector4& Color, float Size, float Rotation) const
{
...
void FRenderer::RenderWorldAxis(const FMatrix& View, const FMatrix& Projection, const FVector4& Color, const FVector& Axis, float Thickness) const
{
...
void FRenderer::RenderWorldGrid(const FMatrix& ViewProjection, const FVector& CameraPosition) const
{
...
UINT FRenderer::GetWidth() const
{
	return Width;
}

UINT FRenderer::GetHeight() const
{
	return Height;
}
